  ...
  return dashboardShell({
    ...
    <div class="card" style="margin-bottom:1.5rem">
      ...
      <div class="table-wrap">
        <table><thead><tr><th>Token</th><th>SOL in</th><th>Entry</th><th>Current</th><th>P&L</th><th>Duration</th><th>Status</th></tr></thead>
        <tbody id="positions-body"><tr><td colspan="7" class="empty-state">Loading...</td></tr></tbody></table>
      </div>
    </div>
...
async function loadPositions(){const p=await api('/snipe/portfolio');const body=$('#positions-body');if(!p||!p.openPositions||!p.openPositions.length){body.innerHTML='<tr><td colspan="7" class="empty-state">No open positions</td></tr>';return;}body.innerHTML=p.openPositions.map(function(pos){const sym=pos.symbol||shortMint(pos.mintAddress);const change=pos.changePct!=null?pos.changePct:0;const st=pos.isMoonBag?'<span class="badge badge-moon">Moon bag</span>':'<span class="badge badge-open">Open</span>';return '<tr><td><div class="token-name">'+sym+'</div><div class="token-mint">'+shortMint(pos.mintAddress)+'</div></td><td style="font-family:var(--mono)">'+solStr(pos.totalSolSpent)+'</td><td style="font-family:var(--mono)">'+usdStr(pos.entryPriceUsd)+'</td><td style="font-family:var(--mono)">'+usdStr(pos.currentPriceUsd)+'</td><td><span class="'+pnlClass(change)+'">'+pnlStr(change)+'</span></td><td style="font-family:var(--mono);color:var(--muted)">'+duration(pos.firstTradeAt)+'</td><td>'+st+'</td></tr>';}).join('');}
